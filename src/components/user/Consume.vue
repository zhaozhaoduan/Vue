...
<div class="page" id="page-infinite-scroll-bottom">
  <div class="buttons-tab">
    <a class="tab-link active button">消费</a>
    <a @click='recharge' class="tab-link button">充值</a>
  </div>
        ...
        <div class="tab active" v-for="item in items">
          <div class="content-block1">
            ...
            <div class="row">
              ...
              <div v-if="item.tradeStatus===1" class="col-80">交易成功</div>
              <div v-if="item.tradeStatus===2" class="col-80">交易等待</div>
              <div v-if='item.tradeStatus===3' class="col-80">交易取消</div>
              <div v-if='item.tradeStatus===4' class="col-80">交易失败</div>
            </div>
          </div>
        </div>
        <!-- 加载提示符 -->
...
export default {
  ready () {
    ...
    $(document).on('pageInit', '#page-infinite-scroll-bottom', function (e, id, page) {
      ...
      that.$http.get('/api/merchants/' + window.user.merchantId + '/users/' + window.user.id + '/cards/' + window.user.cashCardId + '/consumers?type=1&page=' + num).then(function (response) {
  ...
  methods: {
    recharge: function () {
      this.$router.replace({name: 'user_consume_recharge', params: {id: window.card.id}})
    }
  }
}
</script>